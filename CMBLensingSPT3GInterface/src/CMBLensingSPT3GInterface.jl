...
export Parent, Frame, MapSpectraTEB
export @py_str
...
"""
Return an empty FlatSkyMap "parent" with appropriate metadata
that can be populated with pixel values or passed to a MapSpectrum
"""
function Parent(f)
...
function CMBLensing.FlatMap(f::PyObject)
    ...
    FlatMap(py"np.asarray($f)", θpix=maybe_int(py"$f.res / G3Units.arcmin"))
end
...
# flip the fft in the y direction, excluding first row if map Nside is even
function fftflip!(fft::Matrix{Complex{T}}) where T
    fft_flip = @view fft[(1 + size(fft)[2] % 2) : end, :]
    fft_flip .= reverse(fft_flip, dims=1)
    fft
end
function fftflip(f::PyObject)
    @assert pytypeof(f) == py"MapSpectrum2D"
    fftflip!(py"np.asarray($f.get_complex())"[1:end÷2+1,:])
end
fftflip(f::FlatFourier) = fftflip!(unfold(f.Il)[:, 1:end÷2+1])
...
function MapSpectrum2D(f::FlatFourier)
    parent = Parent(f)
    py"MapSpectrum2D($parent, $(fftflip(f)) / get_fft_scale_fac(parent=$parent))"o
end
...
function CMBLensing.FlatFourier(f::PyObject)
    fft = fftflip(f) * py"get_fft_scale_fac(parent=$f.parent)"
    FlatFourier(fft, θpix=maybe_int(py"$f.parent.res / G3Units.arcmin"))
end
...
"""
function MapSpectraTEB(f)
    frame = Frame(Fourier(f), "TEB")
